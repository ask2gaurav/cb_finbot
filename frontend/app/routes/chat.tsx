          ...
          {messages.length === 0 ? (
            ...
            messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-2xl rounded-2xl p-4 ${m.role === 'user' ? 'bg-blue-600 text-white' : m.isError ? 'bg-red-50 shadow border border-red-200 text-red-800' : 'bg-white shadow border border-gray-200 text-gray-800'}`}>
